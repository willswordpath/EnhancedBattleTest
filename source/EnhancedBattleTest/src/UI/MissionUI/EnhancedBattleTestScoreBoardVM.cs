...
namespace EnhancedBattleTest.UI.MissionUI
{
    public class EnhancedBattleTestScoreBoardVM : CustomBattleScoreboardVM
    {
        public override void ExecuteQuitAction()
        {
            ...
            BattleEndLogic.ExitResult exitResult = battleEndLogic?.TryExit() ?? (_mission.IsMissionEnding ? BattleEndLogic.ExitResult.True : BattleEndLogic.ExitResult.NeedsPlayerConfirmation);
            switch (exitResult)
            {
                ...
                case BattleEndLogic.ExitResult.NeedsPlayerConfirmation:
                    OnToggle(false);
                    basicMissionHandler.CreateWarningWidgetForResult(exitResult);
                    break;
                default:
